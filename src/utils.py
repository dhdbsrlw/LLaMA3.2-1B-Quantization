# modified from https://github.com/Nota-NetsPresso/shortened-llm/blob/main/src/utils.py
...
class AttrDict(dict):
    ...
    def from_nested_dicts(cls, data):
        if not isinstance(data, dict):
            return data
        return cls({key: cls.from_nested_dicts(value) for key, value in data.items()})


def build_config(*, cfg_path: str, struct: bool = False) -> AttrDict:
    if not cfg_path:
    ...
    # 2) optional: override from cfg.cfg_path (if present)
    ...
    # 4) convert to AttrDict (recursive)
    container = OmegaConf.to_container(cfg, resolve=True)
    return AttrDict.from_nested_dicts(container)
