...
class TinyStoriesDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        if self.split == "train":
            ...
            return self.process_val_sample(self.data[idx]) # torch.Tensor of shape [num_sequences, seq_len]

    def process_train_sample(self, sample):
        pass
    
    def process_val_sample(self, sample): # single sample
        text = sample["text"]
        # 1. Concatenate all text into a continuous corpus
        corpus_text = "\n\n".join(text)

        # 2. Tokenize into a flat token stream (no special tokens)
        token_stream = self.tokenizer(
            corpus_text,
            return_tensors="pt",
        ...
        token_matrix = token_stream[: num_sequences * self.seq_len]
        sequence_batch = token_matrix.view(num_sequences, self.seq_len) # type: torch.Tensor
